...
docker_helper(){
  ...
  printf "$C_YELLOW Project Building Helpers $C_END \n"
  printf "\t * dh_compose - Run from the project directory and startup the docker-compose compose/local.yml file with necessary flags \n"
  printf "\t * dh_remake - Deletes all project images/containers, and runs $C_YELLOW dh_compose $C_END - will rebuild all images \n"
  printf "\t * dh_remake_cached - Only deletes project containers, and runs $C_YELLOW dh_compose $C_END - will use existing images, if present \n"
  printf "\t * dh_run_tests - Runs the tests (as the CI system will) \n"
...
dh_compose() {
  projectName=${PWD##*/}
  printf "$C_RED Building project: $projectName $C_END \n"
  echo_run "docker-compose -f compose/local.yml -p $projectName build --pull --no-cache"
  echo_run "docker-compose -f compose/local.yml -p $projectName up -d"
}
...
dh_run_tests() {
  projectName=${PWD##*/}
  echo_run "docker-compose -f compose/ci.yml -p $projectName run app bash ./opt/test_app.sh"
}
